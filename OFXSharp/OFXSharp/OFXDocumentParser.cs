...
namespace OFXSharp
{
   public class OFXDocumentParser
   {
      ...
      {
          var list = new List<OFXDocument>();
          foreach (var accountType in GetAccountTypes(ofxString))
          {
              list.Add(ParseAccountType(accountType, ofxString));
          }
          return list.ToArray();
      }


      private OFXDocument ParseAccountType(AccountType accountType, string ofxString)
      {


          var ofx = new OFXDocument { AccType = accountType };

          //Load into xml document
          var doc = new XmlDocument();
          doc.Load(new StringReader(ofxString));

          var xpath = GetXPath(ofx.AccType, OFXSection.CURRENCY);
          var currencyNode = doc.SelectSingleNode(xpath);
      ...
      private AccountType[] GetAccountTypes(string file)
      {
          var list = new List<AccountType>();

          if (file.IndexOf("<BANKMSGSRSV1>") != -1)
              list.Add(AccountType.BANK);
          
          if (file.IndexOf("<CREDITCARDMSGSRSV1>") != -1)
                list.Add(AccountType.CC);
      ...
      private string SGMLToXML(string file)
      {
         ...
         // Thanks to Scott Hanselman for the tip on auto-closing SGML elements : http://www.hanselman.com/blog/PostprocessingAutoClosedSGMLTagsWithTheSGMLReader.aspx
          AutoCloseSgmlElements(reader, xml);
      ...
      void AutoCloseSgmlElements(SgmlReader reader, XmlWriter writer)
      {
          ...
          while (reader.Read())
          {
              switch (reader.NodeType)
              {
                  ...
                  case XmlNodeType.Text:
                      ...
                      else throw new OFXParseException("Unexpected empty text node.");
                      break;
